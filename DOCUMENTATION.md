...
> Документ актуален по состоянию на **март 2026**.
...
## 11. Что реализовано / Что предстоит

### Реализовано

- [x] Аутентификация (guest / agronomist) с sessionStorage
- [x] Защищённые маршруты
- [x] Navbar с активными ссылками
- [x] Dashboard: 4 метрики, мини-карта, погода, donut-диаграмма культур
- [x] Map: интерактивная карта, список полей, fly-to анимация, легенда
- [x] Weather: текущая погода + 7-дневный прогноз по координатам полей
- [x] Реальные координаты полей (3 поля, Краснодарский край)
- [x] Цветовая схема по культурам (единый источник)
- [x] TypeScript-типы для доменных объектов
- [x] Обобщённый хук `useFetch` с отменой

### Предстоит реализовать

- [ ] Страница Calendar — планирование операций по датам
- [ ] Страница Field/:id — детальная карточка поля
- [ ] Добавление новых полей агрономом
- [ ] Форма планирования операции / назначения техники
- [ ] Форма ввода планового урожая
- [ ] Диаграмма урожайности по годам (BarChart)
- [ ] Блок "Последние операции" на дашборде
- [ ] Подключение базы данных (PostgreSQL / SQLite)
- [ ] Авторизация на бэкенде (JWT)
- [ ] Добавление оставшихся полей с реальными координатами
